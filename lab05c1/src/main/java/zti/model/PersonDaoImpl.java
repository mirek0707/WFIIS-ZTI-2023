...
public class PersonDaoImpl implements PersonDao {
    ...
    public int save(Person person) {
        String sql = "INSERT INTO person (fname, lname, email, city, tel) VALUES (?, ?, ?, ?, ?)" ;
        //jdbcTemplate = new JdbcTemplate(dataSource);
        return jdbcTemplate.update(sql, person.getFname(), person.getLname(), person.getEmail(), person.getCity(), person.getTel());
    }

    public int update(Person person) {
        String sql = "UPDATE person SET fname=?, lname=?, email=?, city=?, tel=? WHERE id=?" ;
        //jdbcTemplate = new JdbcTemplate(dataSource);
        return jdbcTemplate.update(sql, person.getFname(), person.getLname(), person.getEmail(), person.getCity(), person.getTel(), person.getId());
    }
